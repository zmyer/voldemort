...
public class StoreDefinitionsMapper {
    ...
    private static StoreDefinition readStore(Element store) {
        ...
        String routingStrategyType = (null != store.getChildText(STORE_ROUTING_STRATEGY)) ? store.getChildText(
    ...
    private Element storeToElement(StoreDefinition storeDefinition) {
        ...
        store.addContent(new Element(STORE_ROUTING_TIER_ELMT).setText(storeDefinition.getRoutingPolicy()
                .toDisplay()));
        store.addContent(new Element(STORE_REPLICATION_FACTOR_ELMT)
                .setText(Integer.toString(storeDefinition.getReplicationFactor())));
        ...
        if (zoneReplicationFactor != null) {
            ...
            for (Integer zone : zoneReplicationFactor.keySet()) {
                zoneReplicationFactorNode.addContent(new Element(STORE_REPLICATION_FACTOR_ELMT)
                        .setText(Integer.toString(zoneReplicationFactor.get(zone)))
        ...
        if (storeDefinition.hasPreferredReads()) {
            store.addContent(new Element(STORE_PREFERRED_READS_ELMT)
                    .setText(Integer.toString(storeDefinition.getPreferredReads())));
        }
        store.addContent(
                new Element(STORE_REQUIRED_READS_ELMT).setText(Integer.toString(storeDefinition.getRequiredReads())));
        if (storeDefinition.hasPreferredWrites()) {
            store.addContent(new Element(STORE_PREFERRED_WRITES_ELMT)
                    .setText(Integer.toString(storeDefinition.getPreferredWrites())));
        }
        store.addContent(
                new Element(STORE_REQUIRED_WRITES_ELMT).setText(Integer.toString(storeDefinition.getRequiredWrites())));
        if (storeDefinition.hasZoneCountReads()) {
            store.addContent(
                    new Element(STORE_ZONE_COUNT_READS).setText(Integer.toString(storeDefinition.getZoneCountReads())));
        }
        ...
        if (storeDefinition.hasHintPreflistSize()) {
            store.addContent(
                    new Element(HINT_PREFLIST_SIZE).setText(Integer.toString(storeDefinition.getHintPrefListSize())));
        }
        ...
        if (storeDefinition.hasRetentionPeriod()) {
            store.addContent(new Element(STORE_RETENTION_POLICY_ELMT)
                    .setText(Integer.toString(storeDefinition.getRetentionDays())));
        }

        if (storeDefinition.hasRetentionScanThrottleRate()) {
            store.addContent(new Element(STORE_RETENTION_SCAN_THROTTLE_RATE_ELMT)
                    .setText(Integer.toString(storeDefinition.getRetentionScanThrottleRate())));
        }

        if (storeDefinition.hasMemoryFootprint()) {
            store.addContent(
                    new Element(STORE_MEMORY_FOOTPRINT).setText(Long.toString(storeDefinition.getMemoryFootprintMB())));
        }
